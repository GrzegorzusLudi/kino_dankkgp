...
npm add @org/ui --workspace

# Or with --filter from anywhere
npm add @org/ui --filter @org/app --workspace
```
...
3. Run install (`npm install`, `npm install`, etc.)
...
- **Hoisting differs by manager:**
  ...
  - npm: no hoisting (strict isolation, prevents phantom deps)
