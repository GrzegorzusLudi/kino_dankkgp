...
- Applications → `apps/<name>`. Check workspace globs (e.g. `npm-workspace.yaml`, `workspaces` in root `package.json`) for an existing `apps/*` entry.
...
### npm Workspace Globs (Critical)

`nx import` adds the imported directory itself (e.g. `apps`) to `npm-workspace.yaml`, **NOT** glob patterns for packages within it. Cross-package imports will fail with `Cannot find module`.
...
- `npm-lock.yaml` — stale; dest has its own lockfile
- `npm-workspace.yaml` — source workspace config; conflicts with dest
...
1. Install ESLint deps first: `npm add -wD eslint@^9 @nx/eslint-plugin typescript-eslint` (plus framework-specific plugins)
...
Install `typescript-eslint` explicitly — npm's strict hoisting won't auto-resolve this transitive dep of `@nx/eslint-plugin`.
...
`@nx/eslint` may peer-depend on ESLint 8, causing the wrong version to resolve. If lint fails with `Cannot read properties of undefined (reading 'allow')`, add `npm.overrides`:

```json
{ "npm": { "overrides": { "eslint": "^9.0.0" } } }
```

### Dependency Version Conflicts

After import, compare key deps (`typescript`, `eslint`, framework-specific). If dest uses newer versions, upgrade imported packages to match (usually safe). If source is newer, may need to upgrade dest first. Use `npm.overrides` to enforce single-version policy if desired.
...
`npm install` fails during `nx import` if a `"workspace:*"` dependency hasn't been imported yet. File operations still succeed. **Fix**: Import all projects first, then `npm install --no-frozen-lockfile`.
...
3. Install test runner deps: `npm add -wD jest jest-environment-jsdom ts-jest @types/jest`
...
When the source is a plain npm/npm workspace without `nx.json`.
...
`nx import` may bring `node_modules/` (npm symlinks pointing to the source filesystem) and `npm-lock.yaml` from the source. Both are stale.

**Fix**: `rm -rf imported/node_modules imported/npm-lock.yaml imported/npm-workspace.yaml imported/.gitignore`, then `npm install`.
