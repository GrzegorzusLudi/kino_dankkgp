...
```
npm add -wD jest ts-jest @types/jest @nx/jest
```
...
```
npm add -wD @testing-library/react @testing-library/jest-dom
```
...
```
npm add -wD babel-jest @babel/core @babel/preset-env @babel/preset-react @babel/preset-typescript
```
...
3. Install deps: `npm add -wD jest jest-environment-jsdom ts-jest @types/jest`
