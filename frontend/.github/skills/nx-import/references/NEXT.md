...
Next.js-specific guidance for `nx import`. For generic import issues (npm globs, root deps, project references, name collisions, ESLint, frontend tsconfig base settings, `@nx/react` typings, Jest preset, target name prefixing, non-Nx source handling), see `SKILL.md`.
...
Next.js detects missing `@types/react` during `next build` and tries to install it using `yarn add` regardless of the actual package manager. In a npm workspace, this fails with a "nearest package directory isn't part of the project" error.

**Root cause**: `@types/react` is missing from root devDependencies.
**Fix**: Install deps at the root before building: `npm add -wD @types/react @types/react-dom`
...
2. Generic fixes from SKILL.md (npm globs, root deps, `.gitkeep` removal, frontend tsconfig base settings, `@nx/react` typings)
3. Install Next.js-specific deps: `npm add -wD @next/eslint-plugin-next`
...
2. Generic fixes from SKILL.md (npm globs, stale files cleanup, script rewriting, target name prefixing)
...
- Errors found & fixed:
  1. npm-workspace.yaml: `apps`/`libs` → `apps/*`/`libs/*`
...
- Errors found & fixed:
  1. npm-workspace.yaml: `apps/web` → `apps/*`
  2. Stale files: `node_modules/`, `npm-lock.yaml`, `npm-workspace.yaml`, `.gitignore` — deleted
...
- Next.js-specific findings:
  ...
  2. Stale files: `node_modules/`, `package-lock.json`, `.gitignore` — deleted (npm workspace, no npm files)
...
- Errors found & fixed:
  ...
  2. Stale files from Vite source: `node_modules/`, `npm-lock.yaml`, `npm-workspace.yaml`, `.gitignore`, `nx.json`
  3. Removed rewritten scripts from Vite app's `package.json`
  4. ESLint 8 vs 9 conflict — `@nx/eslint` peer on ESLint 8 resolved wrong version. Fixed with `npm.overrides`
